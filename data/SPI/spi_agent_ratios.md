# SPI.L Enabled Financial Ratios Analysis

This document contains ONLY the enabled financial ratios with their calculated values and evaluation outcomes.
Data Source: xp-power.md (H1 2025 Results)

## **Current Market Data**

| Metric | Value | Source | Retrieved |
|--------|-------|--------|----------|
| **Share Price** | £2.08 | Yahoo Finance | 2025-09-03 11:37:42 |
| **Market Cap** | £871.1m | Yahoo Finance | 2025-09-03 11:37:42 |
| **Enterprise Value** | £2109.8m | Calculated | - |
| **Shares Outstanding** | 0.0m | Financial Reports | - |

## Valuation Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| P/E Ratio | 17.1% | <10.0 / 10.0-15.0 / >15.0 | FAIL |
| EV/EBITDA | 8.5x | <5.0 / 5.0-7.5 / >7.5 | FAIL |
| EV/EBIT | 15.4x | <7.0 / 7.0-10.0 / >10.0 | FAIL |
| EV/Revenue | 0.7x | >1.0 / 2.0-1.0 / <2.0 | FAIL |
| Price to FCF | 3.5x | >15.0 / 30.0-15.0 / <30.0 | FAIL |
| FCF Yield | 28.4% | >10.0 / 6.0-10.0 / <6.0 | PASS |

## Profitability Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Gross Margin | 45.2% | >40.0 / 30.0-40.0 / <30.0 | PASS |
| Operating Margin | 9.4% | >15.0 / 5.0-15.0 / <5.0 | MONITOR |
| Net Margin | 1.7% | >5.0 / 0.0-5.0 / <0.0 | MONITOR |
| ROE | 6.8% | >10.0 / 0.0-10.0 / <0.0 | MONITOR |
| ROCE | 6.8% | >15.0 / 8.0-15.0 / <8.0 | FAIL |

## Liquidity Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Current Ratio | 0.7% | >1.5 / 1.0-1.5 / <1.0 | FAIL |
| Quick Ratio | 0.5% | >1.0 / 0.5-1.0 / <0.5 | MONITOR |

## Leverage Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Debt-to-Equity | 1.7 | <0.5 / 0.5-1.0 / >1.0 | FAIL |
| Interest Coverage | 0.7x | >4.0 / 2.0-4.0 / <2.0 | FAIL |
| Net Debt/EBITDA | 5.0 | <2.5 / 2.5-3.5 / >3.5 | FAIL |

## Efficiency Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Inventory Turnover | 35.5x | >4.0 / 2.0-4.0 / <2.0 | PASS |
| Days Sales Outstanding | 9 days | <60.0 / 60.0-90.0 / >90.0 | PASS |

## Earnings Quality Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Accruals Ratio | -17.9% | <10.0 / 10.0-20.0 / >20.0 | PASS |
| EBITDA to FCF Conversion | 99.3 | >70.0 / 40.0-70.0 / <40.0 | PASS |
| Adjusted vs Statutory Gap | 0.0 | <10.0 / 10.0-20.0 / >20.0 | PASS |

## Asset Quality Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Goodwill/Assets | 17.6 | <30.0 / 30.0-50.0 / >50.0 | PASS |
| Capex/Depreciation | 0.0 | 0.8-1.2 / <0.8 or >1.2 / >1.5 | MONITOR |
| Tangible Book Value | £308.8m | >50.0 / 0.0-50.0 / <0.0 | PASS |

## Cash Flow Ratios

| Ratio | Value | Threshold (Pass/Monitor/Fail) | Outcome |
|-------|-------|-------------------------------|---------|
| Cash Conversion | 9.3 | >1.0 / 0.8-1.0 / <0.8 | PASS |
| Free Cash Flow | £123.6m | >0.0 / 5.0-0.0 / <5.0 | PASS |

## Summary Statistics

- **Total Enabled Ratios**: 26
- **PASS**: 11 (42%)
- **MONITOR**: 5 (19%)
- **FAIL**: 10 (38%)

---
*This file is generated during analysis execution and contains calculated values with evaluations.*