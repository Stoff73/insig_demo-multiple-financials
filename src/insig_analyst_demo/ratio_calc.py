...
# Add online_extraction to path if needed
online_extraction_path = Path(__file__).parent.parent.parent / 'online_extraction'
if online_extraction_path.exists() and str(online_extraction_path) not in sys.path:
    sys.path.insert(0, str(online_extraction_path))
...
class FinancialRatioCalculator:
    ...
    def __init__(self, company_ticker: str = 'XPP.L', data_dir: str = 'data'):
        ...
        self.json_data = None  # Will hold JSON data if available
        self.using_json = False  # Track if using JSON data
    ...
    def fetch_market_data(self) -> Dict:
        """Get market data from JSON only"""
        # Check for market data in the correct location based on JSON structure
        if self.json_data and 'data' in self.json_data:
            data = self.json_data['data']
            
            # Check if it's year-based structure
            years = sorted([y for y in data.keys() if y.isdigit()], reverse=True)
            market_data = None
            
            if years:
                # Look for market data in the most recent year
                for year in years:
                    year_data = data.get(year, {})
                    if 'market_data' in year_data and year_data['market_data'].get('market_cap'):
                        market_data = year_data['market_data']
                        break
            ...
            if market_data:
                share_price_raw = market_data.get('share_price', 0)
                # Convert pence to pounds for UK stocks
                if self.ticker.endswith('.L'):
                    share_price = share_price_raw / 100
                else:
                    share_price = share_price_raw
                
                return {
                    'share_price': share_price,
                    'market_cap': market_data.get('market_cap', 0) / 1_000_000 if market_data.get('market_cap', 0) > 1000 else market_data.get('market_cap', 0),
                    'company_name': market_data.get('company_name', self.ticker),
                    'currency': market_data.get('currency', 'GBP' if self.ticker.endswith('.L') else 'USD'),
                    'shares_outstanding': market_data.get('shares_outstanding', 0) / 1_000_000 if market_data.get('shares_outstanding', 0) > 1000 else market_data.get('shares_outstanding', 0)
                }
    ...
    def extract_metrics_from_files(self) -> Dict:
        """No longer needed - all data comes from JSON"""
        return {}
    
    def _find_latest_json(self) -> Optional[Path]:
        """Find the {ticker}.json file for the ticker"""
        ticker_base = self.ticker.split('.')[0].upper()
        
        # Check for {ticker}.json file in data/{ticker} folder
        json_file = self.data_dir / ticker_base / f'{self.ticker}.json'
        if json_file.exists():
            return json_file
        
        # Try without exchange suffix
        json_file = self.data_dir / ticker_base / f'{ticker_base}.json'
        if json_file.exists():
            return json_file
        
        # Check multiple possible locations as fallback
        possible_dirs = [
            self.data_dir / self.ticker,  # data/AAPL
            self.data_dir / self.ticker_base,  # data/AAPL (without suffix)
            self.data_dir / f'{self.ticker_base}.L',  # data/AAPL.L
        ]
        ...
        for json_dir in possible_dirs:
            if json_dir.exists():
                ...
                json_files = list(json_dir.glob(f'{self.ticker}*.json'))
                if not json_files:
                    json_files = list(json_dir.glob(f'{self.ticker_base}*.json'))
        ...
        return max(all_json_files, key=lambda x: x.stat().st_mtime)
    
    def _should_update_json(self, json_path: Path) -> bool:
        """Check if JSON needs updating (older than 3 months)"""
        file_modified = datetime.fromtimestamp(json_path.stat().st_mtime)
        three_months_ago = datetime.now() - timedelta(days=90)
        return file_modified < three_months_ago
    
    def _request_data_fetch(self) -> bool:
        """No longer fetches data - JSON must be provided"""
        print(f"JSON file {self.ticker}.json must be provided in data/{self.ticker_base}/ folder")
        return False
    
    
    def _load_json_data(self) -> bool:
        """Load financial data from JSON file only - no external fetching"""
        json_path = self._find_latest_json()
        
        if not json_path:
            # No JSON exists - must be provided
            print(f"ERROR: No JSON data found for {self.ticker}")
            print(f"Please ensure {self.ticker}.json exists in data/{self.ticker_base}/ folder")
            return False
        ...
        try:
            ...
            print(f"Loaded financial data from: {json_path}")
            self.data_sources.append(f"Financial data: {json_path}")
    ...
    def _extract_metrics_from_json(self) -> Dict:
        ...
        # Check if data has year-based structure or direct structure
        years = sorted([y for y in data.keys() if y.isdigit()], reverse=True)
        
        if years:
            # Year-based structure (old format)
            # Use the most recent year with actual financial data (not just market data)
            latest_year = None
            for year in years:
                year_data = data.get(year, {})
                if year_data.get('income_statement') and year_data.get('balance_sheet'):
                    latest_year = year
                    break
            
            if not latest_year:
                print("No financial statement data found in JSON")
                return metrics
            
            print(f"Using financial data from year: {latest_year}")
            latest_data = data.get(latest_year, {})
        else:
            # Direct structure (new format) - financial data is directly under 'data'
            if data.get('income_statement') and data.get('balance_sheet'):
                print("Using financial data from direct structure")
                latest_data = data
            else:
                print("No financial statement data found in JSON")
                return metrics
        
        # Extract income statement metrics (convert from raw values to millions)
        income = latest_data.get('income_statement', {})
        if income:
            metrics['Revenue'] = self._safe_value(income.get('revenue'), 1_000_000)
            metrics['COGS'] = self._safe_value(income.get('cogs'), 1_000_000)
            metrics['Gross Profit'] = self._safe_value(income.get('gross_profit'), 1_000_000)
            # Try both 'operating_profit' and 'operating_income' field names
            operating_profit = income.get('operating_profit', income.get('operating_income', 0))
            metrics['Operating Profit'] = self._safe_value(operating_profit, 1_000_000)
            metrics['Adjusted Operating Profit'] = metrics['Operating Profit']  # Use same value
            metrics['Statutory Operating Profit'] = self._safe_value(
                income.get('statutory_operating_profit', operating_profit), 1_000_000
            )
            metrics['EBIT'] = self._safe_value(income.get('ebit'), 1_000_000)
            metrics['Interest Expense'] = self._safe_value(income.get('interest_expense'), 1_000_000)
            metrics['Net Income'] = self._safe_value(income.get('net_income'), 1_000_000)
            
            # Try to get depreciation from income statement or cash flow
            depreciation = income.get('depreciation')
            if depreciation is None:
                cash_flow_temp = latest_data.get('cash_flow', {})
                depreciation = cash_flow_temp.get('depreciation')
            metrics['Depreciation'] = self._safe_value(depreciation, 1_000_000)
            
            metrics['Amortization'] = self._safe_value(income.get('amortization'), 1_000_000)
            
            # Calculate EBITDA if not directly available
            ebitda_val = self._safe_value(income.get('ebitda'), 1_000_000)
            
            # If not in income statement, check derived_metrics
            if ebitda_val == 0:
                derived = latest_data.get('derived_metrics', {})
                ebitda_val = self._safe_value(derived.get('ebitda'), 1_000_000)
            
            # If still 0, calculate from EBIT
            if ebitda_val == 0 and metrics.get('EBIT'):
                # EBITDA = EBIT + Depreciation + Amortization
                # But if depreciation/amortization are not available, estimate from operating profit
                if metrics.get('Depreciation') or metrics.get('Amortization'):
                    ebitda_val = metrics['EBIT'] + metrics.get('Depreciation', 0) + metrics.get('Amortization', 0)
                else:
                    # Rough estimate: EBITDA typically 10-20% higher than EBIT
                    ebitda_val = metrics['EBIT'] * 1.15
            metrics['EBITDA'] = ebitda_val
            
            # Calculate gross margin if we have the data
            if metrics.get('Revenue') and metrics.get('Revenue') > 0:
                if metrics.get('Gross Profit'):
                    metrics['Gross Margin %'] = (metrics['Gross Profit'] / metrics['Revenue']) * 100
                elif income.get('gross_margin_percent'):
                    metrics['Gross Margin %'] = income.get('gross_margin_percent')
        
        # Extract balance sheet metrics (convert from raw values to millions)
        balance = latest_data.get('balance_sheet', {})
        if balance:
            metrics['Total Assets'] = self._safe_value(balance.get('total_assets'), 1_000_000)
            metrics['Current Assets'] = self._safe_value(balance.get('current_assets'), 1_000_000)
            # Try both 'cash' and 'cash_and_bank_balance' field names
            cash_value = balance.get('cash_and_bank_balance', balance.get('cash', 0))
            metrics['Cash and Bank Balances'] = self._safe_value(cash_value, 1_000_000)
            # Try both 'inventories' and 'inventory' field names
            inventory_value = balance.get('inventories', balance.get('inventory', 0))
            metrics['Inventories'] = self._safe_value(inventory_value, 1_000_000)
            metrics['Receivables'] = self._safe_value(balance.get('receivables'), 1_000_000)
            metrics['Total Liabilities'] = self._safe_value(balance.get('total_liabilities'), 1_000_000)
            metrics['Current Liabilities'] = self._safe_value(balance.get('current_liabilities'), 1_000_000)
            metrics['Total Debt'] = self._safe_value(balance.get('total_debt'), 1_000_000)
            
            # Try to get net debt from balance sheet or derived metrics
            net_debt = balance.get('net_debt')
            if net_debt is None:
                derived = latest_data.get('derived_metrics', {})
                net_debt = derived.get('net_debt')
            metrics['Net Debt'] = self._safe_value(net_debt, 1_000_000)
            
            metrics['Total Equity'] = self._safe_value(balance.get('total_equity'), 1_000_000)
            metrics['Goodwill'] = self._safe_value(balance.get('goodwill'), 1_000_000)
            metrics['Intangible Assets'] = self._safe_value(balance.get('intangible_assets'), 1_000_000)
            metrics['Tangible Book Value'] = self._safe_value(balance.get('tangible_book_value'), 1_000_000)
        
        # Extract cash flow metrics (convert from raw values to millions)
        cash_flow = latest_data.get('cash_flow', {})
        if cash_flow:
            metrics['Operating Cash Flow'] = self._safe_value(cash_flow.get('operating_cash_flow'), 1_000_000)
            # Capital expenditure is usually negative, make it positive for calculations
            # Try both 'capital_expenditure' and 'capex' field names
            capex = cash_flow.get('capital_expenditure', cash_flow.get('capex', 0))
            capex = self._safe_value(capex, 1_000_000)
            metrics['Capital Expenditure'] = abs(capex) if capex else 0
            metrics['Free Cash Flow'] = self._safe_value(cash_flow.get('free_cash_flow'), 1_000_000)
        
        # Get market data - check if we need fresh data or can use cached
        # Check if market data is at the root level (new format) or in year data (old format)
        if 'market_data' in data:
            # New format - market data at root level
            market_data_from_json = data.get('market_data', {})
        elif years:
            # Old format - market data in year structure
            current_year_data = data.get(years[0], {})  # Most recent year
            market_data_from_json = current_year_data.get('market_data', {})
        else:
            market_data_from_json = {}
        
        if market_data_from_json and 'market_cap' in market_data_from_json:
            # We have market data in JSON - check if it's recent enough
            json_timestamp = self.json_data.get('timestamp', '')
            if json_timestamp:
                from datetime import datetime
                try:
                    json_date = datetime.fromisoformat(json_timestamp.replace('Z', '+00:00'))
                    hours_old = (datetime.now() - json_date.replace(tzinfo=None)).total_seconds() / 3600
                    
                    # If data is less than 24 hours old, use it
                    if hours_old < 24:
                        print(f"Using cached market data from JSON (age: {hours_old:.1f} hours)")
                        metrics['market_cap'] = self._safe_value(market_data_from_json.get('market_cap'), 1_000_000)
                        # Handle share price - for UK stocks (.L suffix), it's usually in pence
                        share_price_raw = market_data_from_json.get('share_price', 0)
                        # For UK stocks, if ticker ends with .L, assume pence and convert to pounds
                        if self.ticker.endswith('.L'):
                            metrics['share_price'] = share_price_raw / 100  # Convert pence to pounds
                        else:
                            metrics['share_price'] = share_price_raw
                        metrics['shares_outstanding'] = self._safe_value(market_data_from_json.get('shares_outstanding'), 1_000_000)
                        metrics['enterprise_value'] = self._safe_value(market_data_from_json.get('enterprise_value'), 1_000_000)
                        
                        # Add default values if missing
                        metrics['company_name'] = self.ticker
                        metrics['currency'] = 'GBP'
                    else:
                        # Data is stale but we can't fetch fresh - use what we have
                        print(f"Market data is {hours_old:.1f} hours old, using existing data...")
                        metrics['market_cap'] = self._safe_value(market_data_from_json.get('market_cap'), 1_000_000)
                        share_price_raw = market_data_from_json.get('share_price', 0)
                        if self.ticker.endswith('.L'):
                            metrics['share_price'] = share_price_raw / 100  # Convert pence to pounds
                    ...
                    # If timestamp parsing fails, use the JSON data anyway
                    print("Using market data from JSON (timestamp parsing failed, but data available)")
                    metrics['market_cap'] = self._safe_value(market_data_from_json.get('market_cap'), 1_000_000)
                    share_price_raw = market_data_from_json.get('share_price', 0)
                    if self.ticker.endswith('.L'):
                        metrics['share_price'] = share_price_raw / 100  # Convert pence to pounds
            ...
            # No market data in JSON, must be provided
            print("WARNING: No market data found in JSON file")
            print(f"Please ensure {self.ticker}.json contains market_data section")
            # Use empty values
            metrics['market_cap'] = 0
            metrics['share_price'] = 0
            metrics['shares_outstanding'] = 0
            metrics['enterprise_value'] = 0
            metrics['company_name'] = self.ticker
            metrics['currency'] = 'USD'
        
        # Use LTM (Last Twelve Months) values for annual figures
        # Since we have full year data, use it directly
        metrics['LTM Adjusted EBITDA'] = metrics.get('EBITDA', 0)
        
        return metrics
    
    def _safe_value(self, value, divisor: float = 1) -> Optional[float]:
        """Safely convert value to float, handling 'value not given' strings"""
        if value is None:
            return 0
        if isinstance(value, str) and 'value not given' in value:
            return 0
        try:
    ...
    def calculate_ratios(self) -> Dict:
        ...
        if self._load_json_data():
            ...
            json_metrics = self._extract_metrics_from_json()
            self.metrics.update(json_metrics)
            print("Using financial data from JSON")
        else:
            # No JSON data available - cannot proceed
            print("ERROR: Cannot calculate ratios without JSON data")
            print(f"Please provide {self.ticker}.json in data/{self.ticker_base}/ folder")
            return {}
